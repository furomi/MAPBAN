...
var activeMaps = ['OREGON', 'CLUBHOUSE', 'CONSULATE', 'BORDER', 'KAFE', 'LAB', 'SKYSCRAPER', 'CHALET', 'BANK'];
var allMaps = ['OREGON', 'CLUBHOUSE', 'CONSULATE', 'BORDER', 'KAFE', 'LAB', 'SKYSCRAPER', 'CHALET', 'BANK'];
var jbmevidroTournamentMaps = ['OREGON', 'CLUBHOUSE', 'CONSULATE', 'BORDER', 'KAFE', 'LAB', 'SKYSCRAPER', 'CHALET', 'BANK'];
var pickSide = ['ATK', 'DEF'];
var mapPool = [];
var allowBan = false;
...
client.on('message', message => {
    if (message.content.toLowerCase() === '!mapveto' || message.content.toLowerCase() === '!map veto' || message.content.toLowerCase() === '!veto') {
        message.reply('!veto bo1  OR  !veto bo3 を入力しマップバンを開始してください。');
